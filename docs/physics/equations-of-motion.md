...
#### Free FIP with friction
The coupled Euler-Lagrange equations for the system, considering the relative frictional torque, are:

1. For $\theta$:
   $$
   \left( \frac{1}{3} m_1 l^2 + m_2 l^2 + J\right) \ddot{\theta} + \left( \frac{1}{2} m_1 + m_2 \right)g l \sin\theta = b (\dot{\theta} - \dot{\phi})
   $$

2. For $\phi$:
   $$
   J \ddot{\phi} = -b (\dot{\theta} - \dot{\phi})
   $$
#### Actuated FIP
